...
async function load() {
  ...
  try {
    ...
    const loadingSelectionVersion=++selectionVersion;
    ...
    $('node-card').removeAttribute('aria-busy');
    ...
    await status();
    if(version!==datasetVersion)return;
    if(data.nodes.length&&selectionVersion===loadingSelectionVersion){
...
function filterGraph() {
  ...
  if(selected&&!allowed.has(selected)){selected=null;++selectionVersion;$('node-card').removeAttribute('aria-busy');$('node-card').replaceChildren(el('div','empty','Выберите участника из отфильтрованной сети.'));}
  layoutNeighborhood();
...
function draw() {
  ...
  if(focus&&selected){
    ctx.font='10px system-ui';ctx.fillStyle='#60776a';ctx.textAlign='left';
